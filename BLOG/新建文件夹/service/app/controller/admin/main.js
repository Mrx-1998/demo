...
class MainController extends Contoller{
  ...
  async addArticle() {
    ...
    let id = this.ctx.request.body.type_id

    let sql = 'UPDATE type set ord_id = ord_id + 1 WHERE id = ' + id

    const res = await this.app.mysql.query(sql)
    const result = await this.app.mysql.insert('article',tmpArticle)
    const insertSuccess = res.affectedRows === 1
    const addOrdSuccess = result.affectedRows === 1
  ...
  async delArticle() {
    let str = this.ctx.params.id
    const id = str.split('+')

    let sql = 'UPDATE type set ord_id = ord_id - 1 WHERE id = ' + id[1]

    const result = await this.app.mysql.query(sql)
 
    const res = await this.app.mysql.delete('article',{'id':id})
    if(res.affectedRows === 1){
      this.ctx.body={data: res, result}
    }else{
  ...
  async addAbout() {
    ...
    const res = await this.app.mysql.update('about', {id: 1, ...temp})
